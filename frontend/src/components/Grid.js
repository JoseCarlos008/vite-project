...
const Grid = ({ users, setUsers, setOnEdit }) => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await axios.delete("http://127.0.0.1:3001/" + id);
...
const GridProduct = ({ products, setProducts, setOnEdit }) => {
  // Função para lidar com a ação de edição
  const handleEdit = (item) => {
    setOnEdit(item); // Define o item para edição
  };
  ...
  const handleDelete = async (cod) => {
    try {
      // Realiza uma requisição DELETE para o servidor para excluir o usuário com o ID especificado
      const response = await axios.delete("http://127.0.0.1:3001/" + cod);
      
      // Filtra o usuário excluído do array de usuários
      const newArray = products.filter((product) => product.cod !== cod);
  ...
  return (
    ...
    <Table>
      ...
      <Tr key={i}>
        {/* Renderiza o nome do usuário */}
        <Td width="45%">{item.nome}</Td>
        {/* Renderiza o email do usuário */}
        <Td width="45%">{item.descricao}</Td>
        ...
        {/* Ícone de lixeira com manipulador de evento onClick para acionar a função handleDelete */}
        <Td alignCenter width="5%">
          <FaTrash onClick={() => handleDelete(item.cod)} />
        </Td>
      </Tr>
     ))
      ) : (
